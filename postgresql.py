...
class ReconnectingPostgreSQLConnectionPool(adbapi.ConnectionPool):
    def _runInteraction(self, interaction, *args, **kw):
        try:
            ...
            transient_errors = ("08003", "08006", "40001", "57014", "53300")
...
class Output(cowrie.core.output.Output):
    ...
    def simpleQuery(self, sql, args):
        ...
        d = self.db.runOperation(sql, args)
        d.addErrback(self.sqlerror)

    @defer.inlineCallbacks
    def write(self, event):
        if event["eventid"] == "cowrie.session.connect":
            r = yield self.db.runQuery("SELECT id FROM sensors WHERE ip = %s", (self.sensor,))
            if r:
                sensorid = r[0][0]
            else:
                yield self.db.runOperation("INSERT INTO sensors (ip) VALUES (%s)", (self.sensor,))
            ...
            self.simpleQuery("INSERT INTO params (session, arch) VALUES (%s, %s)", (event["session"], event["arch"]))
            ...
            r = yield self.db.runQuery("SELECT id FROM clients WHERE version = %s", (event["version"],))
            if r:
                clientid = int(r[0][0])
            else:
                yield self.db.runOperation("INSERT INTO clients (version) VALUES (%s)", (event["version"],))
                r = yield self.db.runQuery("SELECT LASTVAL()")
                clientid = int(r[0][0])
            self.simpleQuery("UPDATE sessions SET client = %s WHERE id = %s", (clientid, event["session"]))
            ...
            self.simpleQuery("UPDATE sessions SET endtime = TO_TIMESTAMP(%s) WHERE id = %s", (event["time"], event["session"]))
        elif event["eventid"] == "cowrie.log.closed":
            self.simpleQuery("INSERT INTO ttylog (session, ttylog, size) VALUES (%s, %s, %s)", (event["session"], event["ttylog"], event["size"]))
            ...
            self.simpleQuery(
                "INSERT INTO ipforwards (session, timestamp, dst_ip, dst_port) VALUES (%s, TO_TIMESTAMP(%s), %s, %s)",
                (event["session"], event["time"], event["dst_ip"], event["dst_port"]),
            )
        elif event["eventid"] == "cowrie.direct-tcpip.data":
            self.simpleQuery(
                "INSERT INTO ipforwardsdata (session, timestamp, dst_ip, dst_port, data) VALUES (%s, TO_TIMESTAMP(%s), %s, %s, %s)",
                (event["session"], event["time"], event["dst_ip"], event["dst_port"], event["data"]),
            )
